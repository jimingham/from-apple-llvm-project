...
void ExpressionVariable::TransferAddress(bool force) {
  ...
  if (force || (m_frozen_sp->GetLiveAddress() == LLDB_INVALID_ADDRESS)) {
    ...
    lldb::addr_t static_addr = m_frozen_sp->GetPointerValue(&addr_type);
    if (addr_type != eAddressTypeLoad)
      return;

    lldb::addr_t dynamic_addr= dyn_sp->GetPointerValue(&addr_type);
    if (addr_type != eAddressTypeLoad || static_addr == dynamic_addr)
        return;
    ...
    if (cur_value != static_addr) {
      LLDB_LOG(log, "Stored value: {0} read from {1} doesn't "
               "match static addr: {2}",
               cur_value, live_addr, static_addr);
      return;
    }
    
    if (!process_sp->WritePointerToMemory(live_addr, dynamic_addr, error)) {
      LLDB_LOG(log, "Got error: {0} writing dynamic value: {1} to {2}",
               error, dynamic_addr, live_addr);
      return;
    }
  }
}
